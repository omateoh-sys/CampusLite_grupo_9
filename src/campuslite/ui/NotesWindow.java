...
public class NotesWindow extends JFrame {
    ...
    private void onCursoSeleccionado() {
        ...
        Course curso = getCursoSeleccionado();
        if (curso == null) return;
        for (Registration ins : gestor.getInscripcionesPorCurso(curso.getCode())) {
            Student est = ins.getEstudiante();
            cbEstudiante.addItem(est.getStudentId() + " — " + est.getFullName());
        }
    ...
    private void reconstruirPanelNotas() {
        ...
        Course curso = getCursoSeleccionado();
        Registration ins = getInscripcionSeleccionada();
        ...
        for (Evaluation ev : evals) {
            ...
            String tipo = ev instanceof Laboratory ? "Lab"
                        : ev instanceof Project    ? "Proy"
                        : "Exam";
            ...
            double notaActual = ins.getGrade(ev.getNombre());
            if (notaActual >= 0)
    ...
    private void accionGuardarNotas() {
        ...
        String carnet     = ins.getEstudiante().getStudentId();
        String codCurso   = curso.getCode();
        int errores = 0;
        int guardadas = 0;
        ...
        for (java.util.Map.Entry<String, JTextField> entry : camposNota.entrySet()) {
            ...
            try {
                ...
                gestor.registerGrade(carnet, codCurso, nomEval, nota);
                guardadas++;
