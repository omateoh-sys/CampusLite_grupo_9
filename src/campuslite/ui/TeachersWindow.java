...
import campuslite.domain.Teacher;
 ...
 */
public class TeachersWindow extends JFrame {
    ...
    private void accionGuardar() {
        ...
        try {
            Teacher m = new Teacher(cod, nom, txtCorreo.getText().trim(), txtEspecialidad.getText().trim());
    ...
    private void cargarTabla() {
        ...
        for (Teacher m : gestor.getTeachers()) {
            long cursos = gestor.getCourses().stream()
                .filter(c -> c.getMaestro() != null &&
                             c.getMaestro().getEmployeeCode().equals(m.getEmployeeCode()))
                .count();
            modeloTabla.addRow(new Object[]{
                m.getEmployeeCode(), m.getFullName(),
                m.getEmail(), m.getSpecialty(), cursos + " curso(s)"
            });
        }
    }
