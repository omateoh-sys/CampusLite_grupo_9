...
public class ReportWindow extends JFrame {
    ...
    private JPanel crearTabPorCurso() {
        ...
        for (Course c : gestor.getCourses())
            cbCurso.addItem(c.getCode() + " — " + c.getName());
        toolbar.add(cbCurso);
    ...
    private void generarReportePorCurso(String codigoCurso,
        ...
        Course curso = gestor.buscarCursoPorCodigo(codigoCurso);
        ...
        for (Registration ins : gestor.getInscripcionesPorCurso(codigoCurso)) {
            Object[] fila = new Object[2 + curso.getEvaluations().size() + 2];
            fila[0] = ins.getEstudiante().getStudentId();
            fila[1] = ins.getEstudiante().getFullName();
            int col = 2;
            for (Evaluation ev : curso.getEvaluations()) {
                double nota = ins.getGrade(ev.getNombre());
                fila[col++] = nota >= 0 ? String.format("%.1f", nota) : "—";
            }
            double prom = ins.calcularPromedioFinal();
            fila[col]   = String.format("%.2f", prom);
            fila[col+1] = ins.isPassing() ? "Aprobado" : "Reprobado";
            modelo.addRow(fila);
        }
    ...
    private JPanel crearTabPorEstudiante() {
        ...
        for (Student e : gestor.getStudents())
            cbEst.addItem(e.getStudentId() + " — " + e.getFullName());
        toolbar.add(cbEst);
        JButton btnGenerar = new JButton("Generar reporte");
        toolbar.add(btnGenerar);
        ...
        btnGenerar.addActionListener(e -> {
            ...
            modelo.setRowCount(0);
            List<Registration> inscripciones = gestor.getInscripcionesPorEstudiante(carnet);
            double sumaProm = 0;
            int aprobados = 0;

            for (Registration ins : inscripciones) {
                Course curso = ins.getCurso();
                for (Evaluation ev : curso.getEvaluations()) {
                    double nota  = ins.getGrade(ev.getNombre());
    ...
    private JPanel crearTabPorSalon() {
        ...
        btnGenerar.addActionListener(e -> {
            ...
            for (Registration ins : gestor.getInscripciones()) {
                boolean aprobado = ins.isPassing();
                ...
                modelo.addRow(new Object[]{
                    ...
                    ins.getCurso().getCode() + " — " + ins.getCurso().getName(),
